...
if __name__ == "__main__":
    main()
#створюємо вікно для користувача

root = tk.Tk()
root.title("Пошук статусів NPG")

# Елементи управління
label = tk.Label(root, text="Введіть ключ або значення поля 'status code':")
label.pack()

entry = tk.Entry(root)
entry.pack()

search_button = tk.Button(root, text="Пошук", command=search_and_display)
search_button.pack()
